locals {
  ...
  # Every Spring service in a pod authenticates to its peers as the pod service client,
  # so its secret is bound here once rather than listed per service. The environment
  ...
  s2s_secret = [
    {
      ...
      valueFrom = "${local.secret_arns.sso}:UAA_STORE_POD_SECRET::"
    },
  ]
...
# Generated by the bootstrap stack and shared with store-core: the pod service client's
# secret, and cua's remember-me and crypto keys.
