...
| `/<project>/<env>/sso` | uaa's four client secrets, both remember-me keys, both crypto keys | generated once by a Lambda in the stack |

Every Spring service gets its layer's service-client secret automatically; uaa, the
gateway, tenancy and cua bind the rest. uaa writes the client secrets into its database
